...
function setupModals() {
  ...
  if (projectFilterSelect) {
    ...
    const multiClassForm = document.getElementById("multiClassForm");
    if (multiClassForm) {
      multiClassForm.addEventListener("submit", (e) => {
        e.preventDefault();
        const name = document.getElementById("multiClassName").value.trim();
        const color = document.getElementById("multiClassColor").value;

        if (!name) return;

        const rows = document.querySelectorAll(
          "#multiClassRowsContainer > div",
        );
        let addedCount = 0;

        rows.forEach((row) => {
          const checkbox = row.querySelector(".multi-day-checkbox");
          if (checkbox && checkbox.checked) {
            const dayId = parseInt(checkbox.value);
            const startTime = row.querySelector(".multi-start-time").value;
            const endTime = row.querySelector(".multi-end-time").value;
            const room = row.querySelector(".multi-room").value.trim();
        ...
        if (addedCount === 0) {
          alert("Por favor, selecciona al menos un día para la asignatura.");
          return;
        }
